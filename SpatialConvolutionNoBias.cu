...
__global__ void im2col_kernel_nobias(const int n, const float* data_im,
...
void im2col_nobias(cudaStream_t stream, const float* data_im, const int channels,
    const int height, const int width, const int ksize_h, const int ksize_w, const int pad_h,
  ...
  // Launch
  im2col_kernel_nobias <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>> (
...
__global__ void col2im_kernel_nobias(const int n, const float* data_col,
...
void col2im_nobias(cudaStream_t stream, const float* data_col, const int channels,
  ...
  col2im_kernel_nobias <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>> (
...
static int culstmextra_SpatialConvolutionNoBias_updateOutput(lua_State *L) {
 ...
 // THCudaTensor *bias = (THCudaTensor*)luaT_getfieldcheckudata(L, 1, "bias", "torch.CudaTensor");
  ...
  for (int elt = 0; elt < batchSize; elt ++) {
    ...
    // This is for bias: Do GENoBias (note: this is a bit confusing because gemm assumes column-major matrices)
    /*THCudaBlas_gemm(
        state,
        't', 'n',
        n_, m_, k_,
    ...
    // Do GENoBias (note: this is a bit confusing because gemm assumes column-major matrices)
    THCudaBlas_gemm(
        state,
        'n', 'n',
        n, m, k,
...
static int culstmextra_SpatialConvolutionNoBias_updateGradInput(lua_State *L) {
  THCState *state = getCutorchState(L);
  // Inputs
  THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
...
static int culstmextra_SpatialConvolutionNoBias_accGradParameters(lua_State *L) {
  THCState *state = getCutorchState(L);
  // Inputs
  THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
  THCudaTensor *gradOutput = (THCudaTensor *)luaT_checkudata(L, 3, "torch.CudaTensor");
...
static const struct luaL_Reg culstmextra_SpatialConvolutionNoBias__ [] = {
  {"SpatialConvolutionNoBias_updateOutput", culstmextra_SpatialConvolutionNoBias_updateOutput},
  {"SpatialConvolutionNoBias_updateGradInput", culstmextra_SpatialConvolutionNoBias_updateGradInput},
  {"SpatialConvolutionNoBias_accGradParameters", culstmextra_SpatialConvolutionNoBias_accGradParameters},
  {NULL, NULL}
};
...
void culstmextra_SpatialConvolutionNoBias_init(lua_State *L)
{
  ...
  luaT_registeratname(L, culstmextra_SpatialConvolutionNoBias__, "nn");
  lua_pop(L,1);
}
